...
function connect([source, sourcePort], [target, targetPort]) {
  const stream = target.audioContext.createMediaStreamSource(
    source.outputs[sourcePort].stream
  );
...
customElements.define(
  ...
  class WireComponent extends HTMLElement {
    ...
    render({ source: [source, sourcePort], target: [target, targetPort] }) {
      const [left, right] = [
        source.$outputs[sourcePort],
        target.$inputs[targetPort],
      ];
      ...
      `}
        >
          <path
            d=${getPolyline(xx1, yy1, xx2, yy2, this.tension)}
            stroke="${this.color}"
            fill="none"
          />
        </svg>
